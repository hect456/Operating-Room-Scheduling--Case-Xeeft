...
it can be too permissive in the other direction. This is not a theoretical nuance:
...
| $\pi^{\text{ovf}}$ | Per-day penalty for a bed stay crossing the horizon boundary (§6.5) |
...
$\pi^{\text{ovf}}=50$ per day (§4, point 5) is sized to sit between the two things it has to
...
only a fixed index a binary happens to be attached to, so there's no way to write
